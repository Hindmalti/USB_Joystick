...
{
	uint8_t data = msg;
	int endpoint_out1 = endp_list[2].bEndpointAddress;
	int endpoint_out2 = endp_list[3].bEndpointAddress;

	int size = 1;
	int timeout = 50;
	int bytes_out;
	libusb_interrupt_transfer(handle,endpoint_out1,&data,size,&bytes_out,timeout);
	libusb_interrupt_transfer(handle,endpoint_out2,&data,size,&bytes_out,timeout);
